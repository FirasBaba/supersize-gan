...
parser.add_argument('--batch_size', type=int, default=config.batch_size,
                    help='Batch size')
parser.add_argument('--train_input_size', type=int, default=config.lr_size,
                    help='The input size of the low resolution training images')
parser.add_argument('--epochs', type=int, default=config.epochs,
                    help='The number of epochs to train for')
parser.add_argument('--gen_pth_path', type=str, default=None,
                    help='The path to the pre-trained model weights to initialize the generator')
parser.add_argument('--disc_pth_path', type=str, default=None,
                    help='The path to the pre-trained model weights to initialize the discirminator')
parser.add_argument('--folder_name', type=str, default="name_the_folder",
...
train_imgs = []
val_imgs = []
if not os.path.exists(os.path.join(config.data_path, "train_images.pkl")) and \
    not os.path.exists(os.path.join(config.data_path, "val_images.pkl")):
...
train_dataset = CelebDataset(image_list=train_imgs, size=args.train_input_size, transform=config.data_transform, is_train=True,
                              crop_proba=None)
val_dataset = CelebDataset(image_list=val_imgs, size=args.train_input_size, is_train=True)
...
scheduler_G = optim.lr_scheduler.MultiStepLR(optimizer_G, milestones=[6,12,18], gamma=0.5)
scheduler_D = optim.lr_scheduler.MultiStepLR(optimizer_D, milestones=[6,12,18], gamma=0.5)
...
for epoch in range(args.epochs):
    ...
    for i, (low_img, high_img) in enumerate(tldr):
        ...
        lossD_original = criterion(disc_original, torch.ones_like(disc_original) - 0.1 * torch.rand_like(disc_original))
        ...
        tldr.set_postfix(
            epoch=epoch,
            loss_vgg=lv/(i+1), loss_adv=la/(i+1), 
            lossD=ld/(i+1),
        )
    ...
    for i, (low_img, high_img) in enumerate(tqdm(val_loader)):
        ...
        low_img = low_img.to(device)
        with torch.no_grad():
            gen_hr_img = gen(low_img).to("cpu").reshape(-1, 3, args.train_input_size * 4, args.train_input_size * 4).numpy() * 0.5 + 0.5
            high_img = high_img.to("cpu").reshape(-1, 3, args.train_input_size * 4, args.train_input_size * 4).numpy() * 0.5 + 0.5
            low_img = low_img.to("cpu").reshape(-1, 3, args.train_input_size, args.train_input_size).numpy()
        for ith_sample in range(args.batch_size):
            ...
            plt.savefig(f"results/{args.folder_name}/val_image_{ith_sample}.png", bbox_inches='tight', pad_inches=0, format='png')
            plt.close()
        break
