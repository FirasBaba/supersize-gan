...
parser.add_argument('--pretraind_model_path', type=str, default=None,
...
for epoch in range(args.epochs):
    ...
    for i, (low_img, high_img) in enumerate(tldr):
        ...
        lv += lossG.item()
        tldr.set_postfix(
            loss_resnet=0.006*lv/(i+1), epoch=epoch
        )
    if epoch % 10 > -1:
        ...
        for i, (low_img, high_img) in enumerate((val_loader)):
            ...
            low_img = low_img.to(device)
            with torch.no_grad():
                gen_hr_img = gen(low_img).to("cpu").reshape(-1, 3, ss, ss).numpy() * 0.5 + 0.5
            ...
            for ith_sample in range(args.batch_size):
                ...
                plt.savefig(f"resgen/{args.folder_name}/val_image_{ith_sample}.png", bbox_inches='tight', pad_inches=0, format='png')
                plt.close()
            break
        utils.create_directory_if_not_exists(config.model_path)
        torch.save(gen.state_dict(), os.path.join(config.model_path, f"gen_pretrained.pth"))
