...
class UpSampleBlock(nn.Module):
    def __init__(self, scale_factor, in_ch, kernel_size):
        ...
        self.conv = nn.Conv2d(
            in_channels=in_ch,
            out_channels=in_ch * (scale_factor**2),
            kernel_size=kernel_size,
            stride=1,
            padding=1,
        )
...
class Generator(nn.Module):
    def __init__(self, n_res_blocks=16):
        ...
        self.last_conv = nn.Conv2d(
            in_channels=128, out_channels=3, kernel_size=9, stride=1, padding=4
        )
        self.tanh = nn.Tanh()
    ...
    def forward(self, x):
        ...
        out = self.tanh(
            out
        )  # to force it to be in range [-1, 1] like high_img as they will be both fed to vgg for content loss
        return out
